...
async function updateImageFields() {
    ...
    for (let listing of listings) {
        // Assign a real image to the first several listings for demo
        if (i < demoImages.length) {
            listing.image = demoImages[i];
            i++;
        } else if (!listing.image || typeof listing.image !== 'string' || !listing.image.startsWith('http') || listing.image.includes('placehold.co')) {
            listing.image = 'https://placehold.co/400x300?text=No+Image';
        }
        await listing.save();
    }
    console.log('Many listings updated with real images, others with placeholder.');
